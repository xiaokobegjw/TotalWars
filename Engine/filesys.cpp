...
std::vector<DirListNode> GetDirListing(const std::string &pathstring)
{
	...
	hFind = FindFirstFile(utf8_to_wide(dirSpec).c_str(), &FindFileData);
#else
	// Find the first file in the directory.
...
{
#ifdef UNICODE
	bool r = CreateDirectory(utf8_to_wide(path).c_str(), NULL);
#else
...
{
#ifdef UNICODE
	return (GetFileAttributes(utf8_to_wide(path).c_str()) != INVALID_FILE_ATTRIBUTES);
#else
...
{
#ifdef UNICODE
	return !PathIsRelative(utf8_to_wide(path).c_str());
#else
...
{
#ifdef UNICODE
	DWORD attr = GetFileAttributes(utf8_to_wide(path).c_str());
#else
...
#ifdef UNICODE
		if (!DeleteFile(utf8_to_wide(path).c_str())) {
#else
...
#ifdef UNICODE
	if (!RemoveDirectory(utf8_to_wide(path).c_str())) {
#else
...
bool DeleteSingleFileOrEmptyDirectory(const std::string &path)
{
	...
	{
#ifdef UNICODE
		bool did = DeleteFile(utf8_to_wide(path).c_str());
#else
	...
	{
#ifdef UNICODE
		bool did = RemoveDirectory(utf8_to_wide(path).c_str());
#else
...
#ifdef UNICODE
		rename_success = MoveFileEx(utf8_to_wide(tmp_file).c_str(), utf8_to_wide(path).c_str(),
